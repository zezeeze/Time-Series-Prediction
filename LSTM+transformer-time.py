...
def run_experiment(input_length, output_length):
    ...
    class TransformerBlock(tf.keras.layers.Layer):
        ...
        def call(self, inputs, training):
            attn_output, attn_weights = self.att(inputs, inputs, return_attention_scores=True, training=training)
    ...
    transformer_output, attn_weights = transformer_block(residual)
    ...
    result_folder = f'result_{input_length}+{output_length}'
    ...
    plt.plot(loss_history.losses['train'], label='Training Loss')
    plt.plot(loss_history.losses['val'], label='Validation Loss')
    ...
    loss_plot_path = os.path.join(result_folder, 'loss_curve.png')
    loss_data_path = os.path.join(result_folder, 'loss_data.csv')
    plt.savefig(loss_plot_path, dpi=300, bbox_inches='tight')
    pd.DataFrame(loss_history.losses).to_csv(loss_data_path, index_label='Epoch')
    plt.close()
    ...
    def save_attention_weights(X_data, time_data, prefix):
        attn_weights = attention_model.predict(X_data)

        avg_weights = np.mean(attn_weights, axis=(0,1))  # [seq_len, seq_len]
        

        attn_folder = os.path.join(result_folder, 'attention_weights')
        if not os.path.exists(attn_folder):
            os.makedirs(attn_folder)
        
        np.save(os.path.join(attn_folder, f'{prefix}_weights.npy'), attn_weights)
...
for input_len, output_len in input_output_combinations:
    print(f'\n{"="*40}')
    print(f'Running experiment with input:{input_len}, output:{output_len}')
    print(f'{"="*40}')
    run_experiment(input_len, output_len)
